      ...
      <section style={{background:'#0F172A',padding:'80px 24px'}}>
        ...
        <div style={{maxWidth:'900px',margin:'0 auto',overflowX:'auto',borderRadius:'16px',overflow:'hidden',border:'1px solid rgba(255,255,255,0.1)'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:'14px'}}>
            ...
            <tbody>
{[['Document management','Included','ShareFile ' + p(30) + '/user','green','green'],['E-signatures','Unlimited','DocuSign ' + p(25) + '/envelope','green','red'],['Time tracking','Included','Clio ' + p(49) + '/user','green','green'],['Invoicing and payments','Included','Extra module','green','green'],['Client portal','Branded','Generic','green','yellow'],['Real-time messaging','Built-in','Not included','green','red'],['AI assistant','Included in Pro','Not available','green','red'],['Recurring invoices','Automatic','Manual','green','yellow'],['Push notifications','Browser + email','Email only','green','yellow'],['2FA security','Included','Included','green','green'],['Flat monthly pricing','No per-user fees','Per-user pricing','green','red'],['Setup time','20 minutes','Days of training','green','red']].map(([f,u,t,uc,tc],i) => (<tr key={i} style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}><td style={{padding:'14px 24px',color:'#CBD5E1',fontWeight:'500'}}>{f}</td><td style={{padding:'14px 24px',textAlign:'center',background:'rgba(28,100,242,0.08)'}}><span style={{color:uc==='green'?'#34D399':'#FBBF24',fontWeight:'700',fontSize:'13px'}}>{uc === 'green' ? '✓ ' : '— '}{u}</span></td><td style={{padding:'14px 24px',textAlign:'center'}}><span style={{color:tc==='green'?'#6EE7B7':tc==='yellow'?'#FBBF24':'#F87171',fontWeight:'500',fontSize:'13px'}}>{tc === 'green' ? '✓ ' : tc === 'red' ? '✗ ' : '~ '}{t}</span></td></tr>))}            </tbody>
          </table>
        </div>
      </section>
