...
export default function QuickBooksAlternative() {
  return (
    <>
      <Breadcrumbs schemaOnly items={JSON.parse('[{"name": "Compare", "href": "/blog"}, {"name": "QuickBooks Alternative", "href": "/quickbooks-alternative"}]')} /><style>{`
...
`}</style><SiteHeader />
    <main style={{overflowX:'hidden',fontFamily:'system-ui,sans-serif'}}>
      <section style={{textAlign:'center',padding:'64px 20px 48px',maxWidth:'100%',margin:'0 auto'}}>
        <p style={{color:'#1C64F2',fontWeight:700,fontSize:'14px',marginBottom:'12px'}}>QUICKBOOKS ALTERNATIVE</p>
        <h1 style={{fontSize:'clamp(28px,5vw,48px)',fontWeight:900,letterSpacing:'-0.03em',marginBottom:'20px'}}>
          QuickBooks is great for bookkeeping.<br/>
          <span style={{color:'#1C64F2'}}>But your firm needs more.</span>
        </h1>
        <p style={{fontSize:'18px',color:'#64748B',maxWidth:'100%',margin:'0 auto 32px',lineHeight:1.7}}>
          QuickBooks handles invoicing — but what about e-signatures, document management, client portals, and team collaboration? FirmFlow replaces 5 tools with one.
        </p>
      ...
      <section style={{padding:'48px 20px',maxWidth:'100%',margin:'0 auto'}}>
        <h2 style={{fontSize:'24px',fontWeight:800,textAlign:'center',marginBottom:'32px'}}>FirmFlow vs QuickBooks — Feature comparison</h2>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:'14px'}}>
            <thead>
              <tr style={{background:'#F8FAFC'}}>
                ...
                <th style={{padding:'14px 20px',textAlign:'center',fontWeight:600,borderBottom:'1px solid #E2E8F0'}}>QuickBooks</th>
              </tr>
            </thead>
            <tbody>
              {[
      ...
      <section style={{background:'linear-gradient(135deg,#1C64F2,#7C3AED)',padding:'64px 20px',textAlign:'center',marginTop:'48px'}}>
        <h2 style={{fontSize:'28px',fontWeight:800,color:'#fff',marginBottom:'16px'}}>Ready to upgrade from QuickBooks?</h2>
