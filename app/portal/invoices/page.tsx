      ...
      <div style={{display:'flex',minHeight:'calc(100vh - 60px)'}}>
        ...
        <main style={{flex:1,padding:'32px',overflow:'auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'16px',marginBottom:'28px'}}>
            {[
              { label:'Total', value:'$' + totalAmount.toLocaleString(), color:'#1D4ED8' },
              { label:'Pending', value:'$' + pendingAmount.toLocaleString(), color:'#92400E' },
              { label:'Paid', value:'$' + paidAmount.toLocaleString(), color:'#15803D' },
            ].map((stat, i) => (
          ...
          <div style={{background:'#fff',borderRadius:'12px',border:'1px solid #E2E8F0',overflow:'hidden'}}>
            {!invoices?.length ? (
              <div style={{padding:'48px',textAlign:'center',color:'#94A3B8'}}>
                ...
                <p style={{fontSize:'13px'}}>Your invoices will appear here</p>
              </div>
            ) : (
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead>
                  <tr style={{background:'#F8FAFC'}}>
                    ...
                    <th style={{padding:'12px 20px',textAlign:'left',fontSize:'11px',fontWeight:'600',color:'#64748B',textTransform:'uppercase',letterSpacing:'0.07em'}}>Issued</th>
                  </tr>
                </thead>
                <tbody>
                  {invoices.map((inv, i) => (
                    <tr key={i} style={{borderTop:'1px solid #F1F5F9'}}>
                      ...
                      <td style={{padding:'14px 20px',fontSize:'14px',fontWeight:'800',color:'#1D4ED8'}}>${(inv.amount || 0).toLocaleString()}</td>
                      <td style={{padding:'14px 20px'}}>
                        <span style={{padding:'4px 10px',borderRadius:'6px',fontSize:'12px',fontWeight:'600',background:inv.status==='paid'?'#F0FDF4':inv.status==='overdue'?'#FEF2F2':'#FEF3C7',color:inv.status==='paid'?'#15803D':inv.status==='overdue'?'#DC2626':'#92400E'}}>
                          {inv.status === 'paid' ? '✅ Paid' : inv.status === 'overdue' ? '🚨 Overdue' : '⏳ Pending'}
                        </span>
                      </td>
                      <td style={{padding:'14px 20px',fontSize:'13px',color:'#64748B'}}>{inv.due_at ? new Date(inv.due_at).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'}) : '—'}</td>
                      <td style={{padding:'14px 20px',fontSize:'13px',color:'#64748B'}}>{inv.issued_at ? new Date(inv.issued_at).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'}) : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
