    ...
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 60px)'}}>
      ...
      <div style={{padding:'20px 24px',background:'#fff',borderBottom:'1px solid #E2E8F0',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
          <div style={{width:'44px',height:'44px',borderRadius:'12px',background:'linear-gradient(135deg,#1C64F2,#7C3AED)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'20px'}}>
            🤖
          </div>
          <div>
            <h2 style={{fontSize:'16px',fontWeight:'800',color:'#0F172A',margin:'0',letterSpacing:'-0.03em'}}>FirmFlow AI</h2>
            <p style={{fontSize:'12px',color:'#64748B',margin:'0'}}>Powered by Claude · Ask anything about {firmName}</p>
          </div>
        </div>
        {messages.length > 0 && (
          <button
        ...
        {messages.length === 0 ? (
          <div style={{maxWidth:'600px',margin:'0 auto'}}>
            <div style={{textAlign:'center',marginBottom:'32px',paddingTop:'40px'}}>
              <div style={{width:'72px',height:'72px',borderRadius:'20px',background:'linear-gradient(135deg,#1C64F2,#7C3AED)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'32px',margin:'0 auto 16px'}}>
                🤖
              </div>
              <h1 style={{fontSize:'24px',fontWeight:'800',color:'#0F172A',margin:'0 0 8px',letterSpacing:'-0.03em'}}>Hi {userName}! 👋</h1>
              <p style={{fontSize:'15px',color:'#64748B',margin:'0'}}>I'm your AI assistant. I can analyze your firm data, spot trends, and give you actionable insights. Ask me anything!</p>
            </div>

            <div style={{marginBottom:'16px'}}>
              <p style={{fontSize:'13px',fontWeight:'600',color:'#374151',marginBottom:'12px'}}>Try asking:</p>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{display:'flex',justifyContent:msg.role === 'user' ? 'flex-end' : 'flex-start',marginBottom:'12px'}}>
                <div style={{maxWidth:'75%',display:'flex',gap:'10px',alignItems:'flex-start'}}>
                  {msg.role === 'assistant' && (
                    <div style={{width:'32px',height:'32px',borderRadius:'10px',background:'linear-gradient(135deg,#1C64F2,#7C3AED)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px',flexShrink:0,marginTop:'4px'}}>
                      🤖
                    </div>
                  )}
                  <div style={{
              ...
              <div style={{display:'flex',gap:'10px',alignItems:'flex-start',marginBottom:'12px'}}>
                ...
                <div style={{padding:'14px 18px',background:'#fff',border:'1px solid #E2E8F0',borderRadius:'16px 16px 16px 4px',boxShadow:'0 1px 3px rgba(0,0,0,0.06)'}}>
                  <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
                    ...
                    <span style={{fontSize:'13px',color:'#64748B',marginLeft:'8px'}}>Analyzing your data...</span>
                  </div>
                </div>
              </div>
            )}
      ...
      <div style={{padding:'16px 24px',background:'#fff',borderTop:'1px solid #E2E8F0'}}>
        {messages.length > 0 && (
          <div style={{display:'flex',gap:'6px',marginBottom:'10px',overflowX:'auto',paddingBottom:'4px'}}>
            {['Revenue overview', 'Overdue invoices', 'Top clients', 'Recommendations'].map((q, i) => (
              <button
                key={i}
        ...
        <div style={{display:'flex',gap:'10px',alignItems:'flex-end'}}>
          <textarea
            ...
            placeholder="Ask about your firm data... (Enter to send)"
            rows={1}
            disabled={loading}
        ...
        <p style={{fontSize:'11px',color:'#94A3B8',marginTop:'8px',textAlign:'center'}}>FirmFlow AI analyzes your real firm data to provide insights. Powered by Claude.</p>
      </div>
