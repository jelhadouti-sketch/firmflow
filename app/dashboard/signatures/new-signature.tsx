    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'20px'}} onClick={() => setOpen(false)}>
      <div onClick={e => e.stopPropagation()} style={{background:'#fff',borderRadius:'16px',padding:'32px',width:'520px',maxWidth:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)',maxHeight:'90vh',overflowY:'auto'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'24px'}}>
          <div>
            <h2 style={{fontSize:'18px',fontWeight:'800',color:'#0F172A',margin:'0 0 4px'}}>✍ Request signature</h2>
            <p style={{fontSize:'13px',color:'#64748B',margin:'0'}}>Send a document for e-signature</p>
          </div>
        ...
        {!documents.length ? (
          <div style={{padding:'20px',background:'#FEF3C7',borderRadius:'10px',marginBottom:'20px'}}>
            <p style={{fontSize:'13px',color:'#92400E',margin:'0 0 8px',fontWeight:'600'}}>⚠️ No documents available</p>
            <p style={{fontSize:'12px',color:'#92400E',margin:'0'}}>Upload a document first before requesting a signature.</p>
            <a href="/dashboard/documents" style={{display:'inline-block',marginTop:'8px',fontSize:'12px',color:'#1C64F2',fontWeight:'600',textDecoration:'none'}}>Go to Documents →</a>
          </div>
        ) : !clients.length ? (
          <div style={{padding:'20px',background:'#FEF3C7',borderRadius:'10px',marginBottom:'20px'}}>
            <p style={{fontSize:'13px',color:'#92400E',margin:'0 0 8px',fontWeight:'600'}}>⚠️ No clients available</p>
            <p style={{fontSize:'12px',color:'#92400E',margin:'0'}}>Invite a client first before requesting a signature.</p>
          ...
          <>
                  ...
                  {showDocDropdown && (
                    <div style={{position:'absolute',left:0,right:0,top:'100%',marginTop:'4px',background:'#fff',border:'1px solid #E2E8F0',borderRadius:'8px',boxShadow:'0 8px 24px rgba(0,0,0,0.12)',maxHeight:'180px',overflowY:'auto',zIndex:10}}>
                      {filteredDocs.length === 0 ? (
                        <div style={{padding:'12px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>No documents found</div>
                      ) : filteredDocs.map(d => (
            ...
            <div style={{marginBottom:'16px',position:'relative'}} ref={clientRef}>
              <label style={labelStyle}>Client to sign *</label>
              {selectedClient ? (
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 12px',border:'1px solid #1C64F2',borderRadius:'8px',background:'#EFF6FF'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
            ...
            <div style={{marginBottom:'24px'}}>
              <label style={labelStyle}>Personal message to client <span style={{color:'#94A3B8',fontWeight:'400'}}>(optional)</span></label>
              <textarea
                value={message}
