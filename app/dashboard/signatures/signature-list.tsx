...
export default function SignatureList({ signatures }: { signatures: Signature[] }) {
  ...
  async function handleDelete(id: string) {
    if (!confirm(t('sigList.deleteConfirm'))) return
    setDeleting(id)
  ...
  return (
    <div>
      <div style={{padding:'16px 20px',borderBottom:'1px solid #E2E8F0',display:'flex',alignItems:'center',gap:'12px',flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:'200px',position:'relative'}}>
          ...
          <input
            ...
            placeholder={t('sig.searchPlaceholder') || 'Search by document, client name or email...'}
        ...
        <div style={{display:'flex',gap:'6px'}}>
          {['all','pending','signed'].map(s => (
            <button key={s} onClick={() => setFilter(s)} style={{padding:'7px 14px',borderRadius:'8px',border:'1px solid',borderColor:filter===s?'#1C64F2':'#E2E8F0',background:filter===s?'#EFF6FF':'#fff',color:filter===s?'#1D4ED8':'#64748B',fontSize:'12px',fontWeight:'600',cursor:'pointer',textTransform:'capitalize'}}>
              {s === 'all' ? t('docList.all') : s === 'pending' ? t('common.pendingStatus') : t('common.signedStatus')}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{padding:'32px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>{t('sig.noSignatures') || 'No signature requests found'}</div>
      ) : (
        <div>
          {filtered.map((sig, i) => {
            ...
            return (
              <div key={i} style={{padding:'16px 20px',borderBottom:'1px solid #F1F5F9',display:'flex',alignItems:'center',gap:'14px',flexWrap:'wrap',background:isOverdue ? '#FFFBEB' : 'transparent'}}>
                <div style={{width:'40px',height:'40px',borderRadius:'10px',background:isSigned?'#F0FDF4':isOverdue?'#FEF2F2':'#FEF3C7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px',flexShrink:0}}>
                  {isSigned ? <CheckCircle2 size={20} color="#15803D"/> : isOverdue ? <AlertCircle size={20} color="#DC2626"/> : <Clock size={20} color="#92400E"/>}
                </div>
                <div style={{flex:1,minWidth:'180px'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'3px',flexWrap:'wrap'}}>
                    ...
                    <span style={{padding:'2px 8px',borderRadius:'10px',fontSize:'10px',fontWeight:'700',background:isSigned?'#F0FDF4':isOverdue?'#FEF2F2':'#FEF3C7',color:isSigned?'#15803D':isOverdue?'#DC2626':'#92400E'}}>
                      {isSigned ? t('common.signed').toUpperCase() : isOverdue ? t('common.overdue').toUpperCase() : t('common.pending').toUpperCase()}
                    </span>
                  </div>
 ...
 <span style={{fontSize:'12px',color:'#64748B'}}> {sig.signer_name}</span>
                  ...
                  <div style={{display:'flex',gap:'12px',marginTop:'4px',flexWrap:'wrap'}}>
                    <span style={{fontSize:'11px',color:'#94A3B8'}}>{t('sig.requested') || 'Requested'}: {sig.created_at ? new Date(sig.created_at).toLocaleDateString(dateLocale) : '—'}</span>
                    {sig.due_date && <span style={{fontSize:'11px',color:isOverdue?'#DC2626':'#94A3B8',fontWeight:isOverdue?'600':'400'}}>{t('sig.due') || 'Due'}: {new Date(sig.due_date).toLocaleDateString(dateLocale)}</span>}
                    {sig.signed_at && <span style={{fontSize:'11px',color:'#15803D'}}>Signed: {new Date(sig.signed_at).toLocaleDateString(dateLocale)}</span>}
                  </div>
                </div>
                ...
                <div style={{display:'flex',gap:'6px',flexShrink:0,flexWrap:'wrap'}}>
                  ...
                  >
                    {deleting === sig.id ? '...' : <Trash2 size={13}/>}
                  </button>
                </div>
              </div>
            )
