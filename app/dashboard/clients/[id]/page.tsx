...
export default async function ClientDetail({ params }: { params: Promise<{ id: string }> }) {
 ...
 {client.phone && <p style={{fontSize:'13px',color:'#64748B',margin:'2px 0 0'}}> {client.phone}</p>}
              <p style={{fontSize:'12px',color:'#94A3B8',margin:'4px 0 0'}}>Client since {client.created_at ? new Date(client.created_at).toLocaleDateString(dateLocale,{day:'numeric',month:'long',year:'numeric'}) : '—'}</p>
            </div>
            <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
              <Link href="/dashboard/messages" style={{padding:'9px 16px',background:'#EFF6FF',color:'#1C64F2',borderRadius:'8px',textDecoration:'none',fontSize:'13px',fontWeight:'600'}}>Message</Link>
              <Link href="/dashboard/signatures" style={{padding:'9px 16px',background:'#1C64F2',color:'#fff',borderRadius:'8px',textDecoration:'none',fontSize:'13px',fontWeight:'600'}}>Request signature</Link>
              <Link href="/dashboard/invoices" style={{padding:'9px 16px',background:'#F0FDF4',color:'#15803D',borderRadius:'8px',textDecoration:'none',fontSize:'13px',fontWeight:'600'}}>New invoice</Link>
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:'20px'}}>
            ...
            <div style={{background:'#fff',borderRadius:'12px',border:'1px solid #E2E8F0',overflow:'hidden'}}>
              ...
              {!signatures?.length ? (
                <div style={{padding:'24px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>{t('portal.noSigRequests')}</div>
              ) : (
                <div>
                  {signatures.map((sig, i) => (
                    <div key={i} style={{padding:'12px 20px',borderBottom:'1px solid #F1F5F9',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <div>
                        ...
                        <p style={{fontSize:'11px',color:'#94A3B8',margin:'0'}}>{sig.created_at ? new Date(sig.created_at).toLocaleDateString(dateLocale) : '—'}</p>
                      </div>
            ...
            <div style={{background:'#fff',borderRadius:'12px',border:'1px solid #E2E8F0',overflow:'hidden'}}>
              ...
              {!engagements?.length ? (
                <div style={{padding:'24px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>{t('eng.noEngTitle')}</div>
              ) : (
                <div>
            ...
            <div style={{background:'#fff',borderRadius:'12px',border:'1px solid #E2E8F0',overflow:'hidden',gridColumn:'1 / -1'}}>
              ...
              {!invoices?.length ? (
                <div style={{padding:'24px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>{t('dash.noInvoices')}</div>
              ) : (
                <div>
                  {invoices.map((inv, i) => (
                    <div key={i} style={{padding:'12px 20px',borderBottom:'1px solid #F1F5F9',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <div>
                        ...
                        <p style={{fontSize:'11px',color:'#94A3B8',margin:'0'}}>{inv.created_at ? new Date(inv.created_at).toLocaleDateString(dateLocale) : '—'}</p>
                      </div>
