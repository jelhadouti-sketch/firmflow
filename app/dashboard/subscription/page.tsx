  ...
  return (
    <div style={{fontFamily:'system-ui,sans-serif',background:'#F8FAFC',minHeight:'100vh'}}>
      ...
      <header style={{background:'#fff',borderBottom:'1px solid #E2E8F0',padding:'0 32px',height:'60px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
          <a href="/dashboard" style={{fontSize:'13px',color:'#64748B',textDecoration:'none'}}>← Back to dashboard</a>
      ...
      <main style={{maxWidth:'760px',margin:'0 auto',padding:'48px 24px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'24px',marginBottom:'32px'}}>
          ...
          <div style={{background:'#fff',borderRadius:'16px',padding:'32px',border:firm?.plan==='starter'?'2px solid #1C64F2':'1px solid #E2E8F0'}}>
            ...
            {['5 team members','50 documents','25 clients','E-signatures','Time tracking','Client portal'].map((f,i) => (
              <p key={i} style={{fontSize:'13px',color:'#374151',margin:'6px 0',display:'flex',gap:'8px'}}>✅ {f}</p>
            ))}
            {firm?.plan !== 'starter' && (
              <button style={{width:'100%',padding:'12px',background:'#F1F5F9',color:'#475569',borderRadius:'8px',border:'none',fontSize:'14px',fontWeight:'600',marginTop:'20px',cursor:'pointer'}}>
          ...
          {/* PRO */}
          <div style={{background:'#fff',borderRadius:'16px',padding:'32px',border:firm?.plan==='pro'?'2px solid #16A34A':'2px solid #1C64F2',position:'relative',boxShadow:'0 8px 30px rgba(28,100,242,0.1)'}}>
            {firm?.plan !== 'pro' && (
