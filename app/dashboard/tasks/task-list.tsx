...
export default function TaskList({ tasks }: { tasks: Task[] }) {
  ...
  return (
    <div>
      <div style={{padding:'16px 20px',borderBottom:'1px solid #E2E8F0',display:'flex',alignItems:'center',gap:'12px',flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:'200px',position:'relative'}}>
          ...
          <input
            ...
            placeholder="Search by title, assignee, or engagement..."
      ...
      {filtered.length === 0 ? (
        <div style={{padding:'32px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>No tasks found</div>
      ) : (
        <div>
          {filtered.map((task, i) => {
              ...
              <div key={i} style={{display:'flex',alignItems:'center',gap:'12px',padding:'14px 20px',borderBottom:'1px solid #F1F5F9',background:isOverdue ? '#FFFBEB' : 'transparent'}}>
                ...
                <div style={{flex:1,minWidth:'140px'}}>
                  ...
                  <div style={{display:'flex',gap:'8px',alignItems:'center',flexWrap:'wrap'}}>
                    ...
                    {task.due_date && <span style={{fontSize:'11px',color:isOverdue?'#DC2626':'#94A3B8',fontWeight:isOverdue?'600':'400'}}>Due: {new Date(task.due_date).toLocaleDateString('en-GB')}</span>}
                  </div>
                </div>
