    ...
    if (session.mode === 'subscription' && session.metadata?.firm_id) {
      ...
      if (session.metadata.user_id) {
        await supabaseAdmin.from('notifications').insert({
          ...
          type: 'invoice_paid',
          title: '🎉 Plan upgraded!',
          message: 'Your firm has been upgraded to the ' + plan.toUpperCase() + ' plan. Enjoy all premium features!',
      ...
      if (invoice) {
                ...
                <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:600px;margin:0 auto">
                  ...
                  <div style="padding:40px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px">
                    ...
                    <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:24px;margin-bottom:24px">
                      ...
                      <p style="font-size:28px;font-weight:900;color:#15803D;margin:0">${cur.symbol}${(invoice.amount || 0).toLocaleString()}</p>
                    </div>
                    <div style="text-align:center">
                      <a href="${process.env.NEXT_PUBLIC_APP_URL}/portal/invoices" style="display:inline-block;background:#1C64F2;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700">View in portal →</a>
                    </div>
                  </div>
                </div>
              `
        ...
        for (const admin of adminProfiles || []) {
                ...
                html: `
                  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:600px;margin:0 auto">
                    ...
                    <div style="padding:40px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px">
                      ...
                      <p style="font-size:14px;color:#475569;margin:0 0 24px"><strong>${clientProfile?.full_name || 'A client'}</strong> has paid invoice <strong>${invoice.invoice_number}</strong></p>
                      <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:24px;margin-bottom:24px">
                        <div style="font-size:32px;font-weight:900;color:#15803D;margin-bottom:4px">${cur.symbol}${(invoice.amount || 0).toLocaleString()}</div>
                        <div style="font-size:13px;color:#64748B">${invoice.invoice_number} · ${invoice.description || 'Professional services'}</div>
                      </div>
                      <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/invoices" style="display:inline-block;background:#1C64F2;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700">View invoices →</a>
                    </div>
                  </div>
                `
              })
          ...
          await supabaseAdmin.from('notifications').insert({
            ...
            type: 'invoice_paid',
            title: '💰 Invoice paid!',
            message: (clientProfile?.full_name || 'A client') + ' paid ' + (invoice.invoice_number || 'invoice') + ' — ' + cur.symbol + (invoice.amount || 0).toLocaleString(),
